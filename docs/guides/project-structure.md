...
├── api/               # 🔌 Spring Boot REST API 클라이언트
│   ├── client.ts     # fetch 인스턴스 (JWT 자동첨부, 401 처리)
│   ├── auth.ts       # 인증 API (signup, login, me)
│   └── todo.ts       # Todo API (CRUD, 상태 토글)
├── auth/
│   └── token.ts      # 토큰 저장/조회/삭제
└── schemas/           # Zod 스키마
    ├── auth.ts       # ⚠️ 비밀번호는 min(6) — 복잡도 규칙 없음
    └── todo.ts
```

> ⚠️ `types/`와 `hooks/`는 **`lib/` 안이 아니라 프로젝트 루트**에 둡니다.
> (`types/`, `hooks/useTodos.ts`) — 일부 범용 템플릿은 이들을 `lib/` 하위에 두지만 이 프로젝트는 다릅니다.
