...
| 대상 | 조치 |
|---|---|
| `status=TEMP`이고 `created_at`이 24시간 경과 | **파일 삭제 + 행은 Soft Delete** (업로드하다 만 고아 파일) |
| `deleted_at`이 30일 경과 | **파일만 삭제, 행은 유지** (감사 기록 보존) |

> ⚠️ **행을 물리 삭제하지 않는다.** 초안에서는 고아 파일을 "파일 + 행 삭제"로 적었으나 부모 `CLAUDE.md` **절대규칙 5(물리 삭제 금지)**를 위반한다. 디스크를 차지하는 것은 파일이지 행이 아니다.
