...
`(NKB 남음)`은 컨텍스트 truncation 마커가 파일에 그대로 저장된 것이다. **최초 커밋(`f00422b`)부터 이 상태**라 git에서 복구할 온전한 버전은 없다.

### ⚠️ 최초 판단을 정정한다 (2026-09-04 추가 조사)

처음에는 "유실이 아니라 **처음부터 작성되지 않았다**"고 적었다. **이 판단은 틀렸다.**

추가 조사에서 **다른 문서가 잘려나간 장들을 번호와 내용까지 특정해 인용**하고 있음을 확인했다.

| 인용 위치 | 인용 내용 | 가리키는 장 |
|---|---|---|
| `ROADMAP.md:195` | "프로파일별 `ddl-auto`는 `CLAUDE.md` **12장 표** 참조" | 12장 (환경·프로파일) |
| `ROADMAP.md:198` | "`.gitattributes`를 삭제하거나 덮어쓰지 않는다 (`CLAUDE.md` **13장**)" | 13장 (Git·개발 환경) |
| `ROADMAP.md:353` | "OAuth2는 MockMvc로 끝까지 검증할 수 없다 (`CLAUDE.md` **14장**)" | 14장 (테스트 전략) |
| `CLAUDE.md:569` | "`.exceptionHandling(...)` // **아래 11장의** EntryPoint / AccessDeniedHandler" | 11장 나머지 |

문서가 존재하지 않는 장을 번호와 구체적 내용까지 짚어 인용할 수는 없다. **원본에는 11장 나머지와 12·13·14장이 존재했고, 파일에 붙여넣는 과정에서 잘린 것이다.**

**오류의 원인**: "최초 커밋부터 잘려 있었다"는 사실은 *언제 잘렸는지*만 말해줄 뿐 *원본에 내용이 있었는지*는 말해주지 않는다. 커밋 시점의 증거로 작성 여부를 단정한 것이 논리 비약이었다.

### 복원 완료

`CLAUDE.md` v1.14와 `docs/ROADMAP.md` v1.31에서 복원을 마쳤다.

| 문서 | 복원 내용 |
|---|---|
| `CLAUDE.md` | 11장 나머지(Security 필터 예외 처리, `GlobalExceptionHandler` 목록, 로깅 정책) + **12장 환경·프로파일** + **13장 Git·개발 환경** + **14장 테스트 전략** |
| `docs/ROADMAP.md` | Phase 11-3(HTTPS) 완성 + **11-4 Amplify** + **11-5 연동 검증 DoD 12항목** |

복원은 **추측 없이** ① 위 표의 인용 계약 ② 실제 코드·설정 파일 ③ 이미 확정된 결정에만 근거했다. 각 장 머리에 복원 노트를 남겨 원본과 재작성분을 구분할 수 있게 했다.
