...
**2026-08-28(같은 날, Phase 0 작업 중) 반대 방향의 사례도 발견됐다**: 이 문서(1장)와 `docs/ROADMAP.md`의 "아직 남은 것" 표가 "세 저장소 모두 master, 커밋 0건, 원격 미연결, 백엔드/프론트 `.env` gitignore 규칙 없음"이라 서술하고 있었으나, 실제 git 상태를 직접 조회하니 브랜치는 이미 `main`이고 커밋도 존재했으며 백엔드 `.env` gitignore 규칙도 이미 커밋되어 있었다. 즉 **"미완료" 표시도 무조건 믿지 말고 실측하라** — 위 사건과 정반대 방향(완료를 미완료로 오기)의 실수였다. 이번에 실제로 미완료였던 것은 `develop` 브랜치 부재와 원격 push 미실행뿐이었다.
...
- 세 저장소 모두 `main`·`develop` 브랜치가 존재하고 원격(`origin`, 이름 통일 완료)에 연결·push되어 있다(Phase 0 완료, 2026-08-28). `main` ← `develop` ← `feature/{작업명}` 전략에 따라, 새 기능 작업은 `develop`에서 `feature/{작업명}`을 분기해 진행하라.
