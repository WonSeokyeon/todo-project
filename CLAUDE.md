...
> **버전** 1.15 · **최종 수정** 2026-09-04
> **v1.15 변경**: 4장 정리 배치 규칙을 정정했다. v1.13에서 고아 파일(TEMP 24시간 경과)을 "파일·행 삭제"로 적었는데 **부모 CLAUDE.md 절대규칙 5(물리 삭제 금지)를 위반**한다. Phase 12 구현 중 발견했고, 코드는 처음부터 규칙을 따라 Soft Delete로 만들었으므로 **문서만 코드에 맞춰 고쳤다.** `docs/tiptap-image-upload-prompt.md` 9장도 같이 정정했다.
...
- **본문에서 사라진 첨부는 `deleted_at`만 채우고 파일은 즉시 지우지 않는다.** 즉시 지우면 향후 휴지통(`PRD.md` 9장 #3) 복원이 구조적으로 불가능해진다. 물리 파일 삭제는 배치가 유예 기간 뒤에 수행한다.
  - `status=TEMP` + 생성 후 24시간 경과 → **파일 삭제 + 행은 Soft Delete** (업로드하다 만 고아 파일)
  - `deleted_at` 30일 경과 → **파일만 삭제, 행은 유지**

> ⚠️ **정리 배치도 행을 물리 삭제하지 않는다** (v1.15 정정). v1.13에서는 고아 파일을 "파일·행 삭제"로 적었으나 **부모 CLAUDE.md 절대규칙 5("삭제는 모두 Soft Delete, 물리 삭제 금지")를 위반**한다. 디스크를 차지하는 것은 파일이지 행이 아니므로, 파일만 지우면 비용 문제는 해결되고 감사 기록은 남는다. 구현은 처음부터 규칙을 따랐고 문서만 어긋나 있었다.
